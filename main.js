// ✅ main.js - Updated to pass Earth pivot and angle to meteor system
...
const sunMesh = new THREE.Mesh(new THREE.SphereGeometry(5, 64, 64), new THREE.MeshBasicMaterial({ map: sunTexture }));
scene.add(sunMesh);

const coronaGeometry = new THREE.SphereGeometry(5.3, 64, 64);
const coronaMaterial = new THREE.MeshBasicMaterial({ color: 0xffcc66, transparent: true, opacity: 0.12, blending: THREE.AdditiveBlending, side: THREE.BackSide });
...
function createAtmosphere(radius, color, intensity = 1.0) {
  ...
  const atmosphereMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: `varying vec3 vNormal; uniform vec3 glowColor; uniform float intensity; void main() { float glow = pow(0.65 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 2.5); gl_FragColor = vec4(glowColor, 1.0) * glow * intensity; }`,
...
const planetsData = [{ name: 'Earth', size: 1.3, distance: 25, texture: '/textures/earth.jpg', axialTilt: 23.44, inclination: 0.0, orbitalPeriod: 1.0, rotationPeriod: 1.0, orbitDirection: 1, orbitColor: 0x4488ff }];
const pivots = [];
...
planetsData.forEach((planet) => {
  ...
  const material = new THREE.MeshStandardMaterial({ 
    map: texture,
    emissive: 0x112244, // Self-illumination
    emissiveIntensity: 0.4, // Make texture visible
    color: 0xffffff,
    roughness: 0.7,
    metalness: 0.2
  });
  ...
  const orbitMaterial = new THREE.LineBasicMaterial({ color: planet.orbitColor, transparent: true, opacity: 0.4 });
...
// Initialize meteor with current Earth position and angle
...
fastForwardBtn.addEventListener('click', () => {
  ...
  fastForwardBtn.style.opacity = '0.5';
});

resetBtn.addEventListener('click', () => {
  isImpacted = false;
  earthAngle = Math.random() * Math.PI * 2;
  pivots[0].angle = earthAngle;
  
  // Update Earth position immediately
  const x = pivots[0].data.distance * Math.cos(earthAngle);
  const z = pivots[0].data.distance * Math.sin(earthAngle);
  pivots[0].mesh.position.set(x, 0, z);
  
  fastForwardBtn.disabled = false;
  fastForwardBtn.style.opacity = '1';
  resetBtn.classList.add('hidden');
  
  // Reset meteor with new Earth position
  resetMeteorImpact(scene, earthMesh, sunMesh, earthPivot, earthAngle);
});
